...
export function ButtonRow(props){
  ...
  const { onClickCancel, onClickSave, onClickDraft, text1, text2, text3, id, status } = props;

  return (
    <div className='invt_btn_row' id={id}>
      <Button className='invt_btn' text={t(text1 ?? 'page.cancel')} onClick={onClickCancel} />
      {status !== 1 && <Button className='invt_btn' text={t(text2 ?? 'page.draft')} onClick={onClickDraft} />}
