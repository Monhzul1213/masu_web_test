...
import { BarStack, PlainSelect } from "../../all";
...
export function Graph(props) {
  ...
  const tipFormatter = (value, name, props) => [name + ' : ' + formatNumber(value) + currency];

  let id = size?.width >= 400 ? 'rr_large' : 'rr_small';
  let style = { width, height: 360, display: 'flex', alignItems: 'center', justifyContent: 'center' };
  let chartProps = { style, data, bar, tickFormatter, tipFormatter };

  const Chart = () => {
    if(type === 'line') return (<div>line</div>);
  ...
  return (
    <div className='ri_graph_back' id={id}>
      <div className='rr_graph_header'>
        <p className='rr_graph_title'>{t('report.top5_items1')} <span className='rr_graph_sub'>{t('report_review.thousand')}</span></p>
