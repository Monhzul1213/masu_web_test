...
export function Filter(props){
  ...
  useEffect(() => {
    if(size?.width >= 850) setClassH('rp_h_back1');
    else if(size?.width < 850 && size?.width >= 320) setClassH('rp_h_back2');
  ...
  const empProps = { value: emp, setValue: setEmp, data: emps, s_value: 'empCode', s_descr: 'empName', onHide,
    Icon: () => <DynamicAIIcon name='AiOutlineUser' className='mr_cal' />, classBack: 'rep_select_back1',
    className: 'rp_select', dropdownStyle: { marginLeft: -30, minWidth: 150 }, dropdownAlign: { offset: [-30, 5] },
