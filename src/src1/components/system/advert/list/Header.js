...
export function Header(props){
  ...
  const onFocusStatus = async () => {
    if(!states?.length || states?.length === 1){
      ...
      setLoading('status');
      const response = t('advert.statusl');
      let data = [...[{label: t('advert.status') , value: -1 }], ...response];
  ...
  const statProps = { value: status, setValue: onChangeStatus, data: states, s_value: 'value', s_descr: 'label',
    label: t('advert.status'), onFocus: onFocusStatus, loading: loading === 'status', classBack, classLabel, className, bStyle };
