...
export function Filter(props){
  ...
  const onHide = () => {
    ...
    // if(status?.length !== status?.length) status?.forEach(item => query += '&Status=' + item);
  ...
  //     let data = [...response];
  //     setStates(data?.sort((a, b) => a.value - b.value));
  //     setLoading(null);
  //   }
  // }
  ...
  // const maxStatus = status?.length === status?.length ? t('time.all_status') : (status?.length + t('time.some_status'));
  ...
  // const statProps = { value: states, setValue: setStates, data: status, s_value: 'status', s_descr: 'statusName', onHide,
  //   Icon: () => <DynamicAIIcon name='AiOutlineProfile' className='mr_cal' />, classBack: 'rp_select_back2',
  //   className: 'rp_select', dropdownStyle: { marginLeft: -30, minWidth: 180 }, dropdownAlign: { offset: [-30, 5] },
  //   onFocus: onFocusStatus, loading: loading === 'status',maxTag: maxStatus,  placeholder: t('orders.status') };
  return (
