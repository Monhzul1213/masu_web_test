...
export function List(props){
  ...
  useEffect(() => {
    ...
    setColumns([
      ...
      { Header: t('customer.addr'), accessor: 'phone', customStyle: { width: 100 },
      ...
      { Header: t('customer.last_visit'), accessor: 'lastUpdate',customStyle: { width: 150 },
        Cell: props => <div style={{fontSize: 13.5, paddingRight: 15}}>{props.value}</div> },
      { Header: <div style={{textAlign: 'center'}}>{t('customer.visit_total')}</div>, accessor: 'total', customStyle: { width: 120 },
        Cell: props => <div style={{textAlign: 'right', paddingRight: 15}}>{formatNumber(props.value)}</div>},
      { Header: <div style={{textAlign: 'center'}}>{t('customer.total_spent')}</div>, accessor: 'total_spent', customStyle: { width: 150 }, 
        Cell: props => <div style={{textAlign: 'right', paddingRight: 15}}>₮{formatNumber(props.value)}</div>},
      { Header: <div style={{textAlign: 'center'} }>{t('customer.total_balance')}</div> , accessor: 'total_balance' , customStyle: { width: 100 },
      Cell: props => <div style={{textAlign: 'right', paddingRight: 15}}>{formatNumber(props.value)}</div>},
    ]);
    return () => {};
