...
import { ButtonRow, CheckBox, Date, MoneyInput, Time } from '../../../../components/all';

export function ModalSales(props){
  const { visibleSales, closeSales, selected, setData } = props;
    ...
    if(timeValid && salesPrice && dateValid){
      setData(old => old.map((row, index) => {
        if(selected?.original?.invtId === row?.invtId)
          return {...old[index], useSalesPrice: 'Y', salesPrice, salesBeginDate, salesEndDate, salesTimeLimited, salesBeginTime, salesEndTime, salesLabel };
        else
          return row;
      }));
      closeSales();
