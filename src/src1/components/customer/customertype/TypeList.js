...
export function TypeList(props){
  ...
  useEffect(() => {
    ...
    setColumns([
      ...
      { Header: <div style={{textAlign: 'right'}}>{t('inventory.cost')}</div>, accessor: 'cost', customStyle: {minWidth: 80},
        ...
        { Header: <div style={{textAlign: 'right'}}>{t('customer.standardPrice')}</div>, accessor: 'standardPrice', customStyle: {minWidth: 140},
        Cell: props => <div style={{textAlign: 'right', paddingRight: 15}}><Money value={props?.value} fontSize={14} /></div>},
        { Header: <div style={{textAlign: 'right'}}>{t('customer.price')}</div>, accessor: 'price', width: 130, isMoney: true},
      {
        ...
        Cell: ({ row, onClickSalesCheck, onClickSales }) => {
          ...
          return (
            <div style={{ display: 'flex', flexFlow: 'row', alignItems: 'center', textAlign: 'right', width: 200, marginLeft:10 }}>
      ...
      },
      {
        ...
        Cell: ({ row, onClickWholeCheck, onClickWhole }) => {
          ...
          return (
            <div style={{ display: 'flex', flexFlow: 'row', alignItems: 'center', textAlign: 'right', width: 130, marginLeft:10 }}>
  ...
  const tableInstance = useTable( { columns, data, defaultColumn, autoResetPage: false,  initialState: { pageIndex: 0, pageSize: 1000000 , sortBy: [{ id: 'invtName', desc: true }]},
  updateMyData, onClickSales, onClickWhole, onClickWholeCheck, onClickSalesCheck }, useSortBy, usePagination, useRowSelect);
  ...
  return (
    <div >
      ...
      <div style={{overflow: 'scroll'}} >
        <div className='table_scroll' id='paging' style={{marginTop: 10, overflowY: 'scroll', maxHeight, minWidth : 900}}>
              <Table {...tableProps} />
        </div>
      </div>
