...
export function Inventory(props){
  const { visible, closeModal, setItem, setVisible, item, siteId, check} = props;
  const { t } = useTranslation();
  const [data, setData] = useState([]);
  const [name, setName] = useState({ value: '' });
  const [site, setSite] = useState({ value: siteId?.value});
  const [sites, setSites] = useState([]);  
  const [isRemain, setIsRemain] = useState(check);
