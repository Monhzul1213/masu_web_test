...
export function Filter(props){
  ...
  const [states, setStates] = useState([{value: -1, label: t('order.all_status')}]);
  ...
  const onFocusStatus = async () => {
    if(!states?.length || states?.length === 1){
      setStates([{value: -1, label: t('order.all_status')}, ...t('count.status')]);
    }
  }
  ...
  const addProps = { type: 'count', onClickAdd, show: false };
