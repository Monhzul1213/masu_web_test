...
import { PaginationTable, Empty1, Money, TableResize } from '../../all/all_m';
...
export function List(props){
  ...
  useEffect(() => {
    setColumns([
      ...
      { Header: <div style={{textAlign: 'right'}}>{t('report.qty1')}</div>, accessor: 'qty_sign', exLabel: t('report.qty1'), width: 90, minWidth: 60,
        Cell: props => <div style={{textAlign: 'right', paddingRight: 15}}>{formatNumber(props?.value)}</div>,
        Footer: info => {
          const total = React.useMemo(() =>
            info.rows.reduce((sum, row) => row.values.qty_sign + sum, 0),
            [info.rows]  )
