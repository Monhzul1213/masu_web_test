...
import {Filter, List } from '../../components/report/payment'
...
function Screen(props){
  ...
  const getData = async (query, query1, dates) => {
    ...
    setFilter(query);
    setFilter1(query1 ?? '');
    if(dates) setExcelName(t('header./report/report_payment') + ' ' + dates[0]?.format('yyyy.MM.DD') + '-' + dates[1]?.format('yyyy.MM.DD'));
