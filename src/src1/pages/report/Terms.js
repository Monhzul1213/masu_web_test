...
import {Filter, List } from '../../components/report/terms';
...
function Screen(props){
  ...
  const getData = async (query, query1, dates) => {
    ...
    setFilter(query);
    setFilter1(query1 ?? '');
    if(dates) setExcelName(t('header./report/terms') + ' ' + dates[0]?.format('yyyy.MM.DD') + '-' + dates[1]?.format('yyyy.MM.DD'));
...
export const Terms = withSizeHOC(Screen);