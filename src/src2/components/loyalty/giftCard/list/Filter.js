...
export function Filter(props) {
  ...
  const width = showSearch ? 0 : size?.width > 780 ? 462 : size?.width;
  const width1 = !showSearch ? 0 : size?.width > 470 ? 412 : size?.width;
  const classBack = "cou_select_back",
    classLabel = "ih_select_lbl",
    className = "ih_select";
  ...
  const addProps = { type: "giftCard", onClickAdd, show: false };
  const dateProps = {
    label: t("page.date"),
    value: date,
    setValue: setDate,
