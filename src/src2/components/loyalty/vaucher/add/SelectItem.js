...
import { CustomSelect, Overlay } from '../../../../../src1/components/all/all_m';
const { Option } = Select;
...
export function SelectItem(props){
  ...
  return (
    <div className='cs_item'>
      <p className='cs_name'>{item?.name ?? item?.descr ?? item?.firstName?? item?.consumerName}</p>
    </div>
  );
}
...
export function ItemSelect(props){
  ...
  const onSelect = value => {
    ...
    if(exists === -1){
      ...
      else setSearch({ value: null, error: t('voucher.number_max') });
    } else {
      setSearch({ value: null, error: t('voucher.already_added') });
    }
  }
  ...
  const selectProps = { value: search, setValue: onSelect, placeholder: t('voucher.search'), data: items,
