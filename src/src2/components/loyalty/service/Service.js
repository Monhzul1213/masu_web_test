...
import { ButtonRow, Overlay, Error, PlainSelect, IconButton, Input, ModalTitle, Empty2 } from '../../../../src1/components/all/all_m';
import { List } from './List';
import { age } from '../vaucher/add/age';
...
export function Service(props){
  ...
  const nameProps = { value: name, setValue: setName, label: t('page.name'), setError, placeholder: t('voucher.name_phone'), inRow: true  };
  const beginAgeProps = { value: beginAge, setValue: setBeginAge, label: t('voucher.age'), setError, className: className1, classLabel, data: age };
  const endAgeProps = { value: endAge, setValue: setEndAge, label: t('voucher.age'), setError, className: className1, classLabel, data: age };
  const genderProps = { value: gender, setValue: onChangeGender, data: genders, s_value: 'value', s_descr: 'label', label: t('voucher.gender'), onFocus: onFocusGender, className: className2 , classLabel};
  const buyerProps = { value: buyer, setValue: onChangeBuyer, data: buyers, s_value: 'value', s_descr: 'label', label: t('voucher.buyer'), onFocus: onFocusBuyer, className , classLabel};
  const otherBuyerProps = { value: otherBuyer, setValue: onChangeOtherBuyer, data: otherBuyers, s_value: 'value', s_descr: 'label', label: t('voucher.otherBuyer'), onFocus: onFocusOtherBuyer, className: className2, classLabel : 'cou_select_lbl1' };
    ...
    <Modal title={null} footer={null} closable={false} open={visible} centered={true} width={600}>
      ...
      <Overlay loading={loading}>
        <div className='m_back'>
          <ModalTitle icon='MdSupervisorAccount' title={t('voucher.consumer_add')} isMD={true} />
