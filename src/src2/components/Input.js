...
export function Input(props) {
  ...
  const onChange = (e) => {
    ...
    if (notValid)
      setValue({
        value: value?.value,
        error: " " + t("error.valid_character"),
        noLabel: true,
      });
    else if (e?.target?.value?.length > length)
      setValue({
        value: value?.value,
        error: " " + length + t("error.shorter_than"),
      });
    ...
    <div style={inRow ? { flex: 1 } : {}} id={id}>
      ...
      {value?.error && (
        <p className="f_input_error">
          {value?.noLabel ? "" : label} {value?.error}
        </p>
      )}
    </div>
  );
...
export function ValidateInput(props) {
  ...
  const onChange = (e) => {
    ...
    if (notValid)
      ...
      setValue({
        value: value?.value,
        error: " " + max + t("error.shorter_than"),
      });
    else setValue({ value: e.target.value });
    setError && setError(null);
    setEdited && setEdited(true);
