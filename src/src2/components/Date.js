...
export function Date(props) {
  ...
  const style = value?.error
    ? { borderColor: "#e41051", color: "#e41051" }
    : {};
  ...
  return (
    <div style={inRow ? { flex: 1 } : {}}>
      <div className="select_back" style={backStyle}>
        <p className="select_lbl" style={style}>
          {label}
        </p>
        <DatePicker
...
export function Time(props) {
  ...
  const onBlur = () => {
    ...
    if (length !== 0 && length !== 6)
      setValue({ value: value?.value?.replace(/-/g, "0") });
    onTime &&
      onTime(length ? { value: value?.value?.replace(/-/g, "0") } : null);
  };
...
const TimeSelect = (props) => {
  ...
  return (
    <div className="mr_time_back">
      ...
      <Select className="mr_time_select" onChange={setValue} value={value}>
  ...
  const menu = () => {
    return (
      <div className="mr_dropdown">
        ...
        {custom && (
          <div className="mr_times">
            <TimeSelect
              label={t("time.t_start")}
              value={value[0]}
              setValue={(val) => setTime([val, value[1]])}
            />
            <div className="gap" />
            <TimeSelect
              label={t("time.t_end")}
              value={value[1]}
              setValue={(val) => setTime([value[0], val])}
            />
          </div>
        )}
      </div>
