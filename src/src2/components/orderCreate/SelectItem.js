...
import { sendRequest } from "../../../../src/services";
...
export function SelectItem(props) {
  ...
  return (
    <div className="cs_item">
      ...
      <p className="cs_sku">
        {label ?? "SKU"} {item?.sku ?? item?.invtId}
      </p>
    </div>
  );
}
...
export function ItemSelect(props) {
  ...
  const getData = async (value) => {
    if (value?.length > 3) {
      ...
      let response = await dispatch(
        sendRequest(user, token, "Inventory/GetInventory/Custom", filter)
      );
  ...
  const renderItem = (item, index) => {
    let optItem = {
      name: item?.msInventory?.name,
      sku: item?.msInventory?.sku,
    };
    return (
